...
module Input_file(Buf: BufSig with type t = bigstring) = struct
  ...
  let byte_size {contents; _} = A.dim contents

  let get_chunk {blocksize; contents; _} i =
    let len = A.dim contents in
    let block_bytes = blocksize * Buf.Conv.bytes in
    if (i + 1) * block_bytes <= len then
      A.sub contents (i * block_bytes) (block_bytes)
    else
      let ba = A.create B.char B.c_layout (block_bytes) in
      A.fill ba '\x00';
      if i * block_bytes < len then
        A.blit
          (A.sub contents (i * block_bytes) (len mod block_bytes))
  ...
  let get_byte chunk i = A.unsafe_get chunk i
...
module Input_string(Buf: BufSig with type t = string) = struct
  ...
  let get_byte {offset; b} i = Bytes.unsafe_get b (offset + i)
